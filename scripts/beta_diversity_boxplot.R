...
library("vegan")
library("reshape2")
beta_diversity_boxplot <- function(OTU_input, group_index, tax_index, index_method = "bray", plot_method = "PcoA"){
  ...
  physeq1 = phyloseq(OTU, TAX, sampledata)
    ...
    #summary(sample_sums(physeq1))
    {
      if(index_method == "bray"){
        diveristy_distance <- phyloseq::distance(relab_genera, method = "bray")
      }
...
p <- ggplot(df.bray, aes(x=Conditions, y=value, colour=Conditions)) + geom_jitter() + geom_boxplot(alpha=0.6) + ylab(paste(index_method,"_diversity")) +
...
q <- plot_ordination(relab_genera, ord, color = "Group") + geom_point(size=4) + stat_ellipse(aes(group=Group)) + theme_bw()
