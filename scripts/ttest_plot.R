...
library("scales")
library("ggplotify")
ttest_plot <- function(OTU_input, group_index, index_pvalue, group1, group2, plot_method, alpha){
...
text_wilcoxtest_summary<- paste("Total of ", nrow(res_tax_sig), " taxa were identified as significant.", sep="")
  ...
  #print("No. significant were identified")
   ...
   {
   p3<-ggplot(data = res_tax, aes(x = as.numeric(All_mean_relative_frequency), y = as.numeric(log2FC), color = Significant)) +
   geom_point(size = 2) +
   scale_x_log10() +
