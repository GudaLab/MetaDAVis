...
deseq2_summary <- function(OTU_input, group_index, index_pvalue, group1, group2, plot_method, alpha, deseq2_color_palette){
...
colors <- colorRampPalette(brewer.pal(9, deseq2_color_palette))(length(unique_conditions))
...
if(plot_method == "1"){
   ...
   {
   p1 <- ggplot(data = res_tax, aes(x = Basemean, y = log2FC, color = Significant)) +
    geom_point(size = 2) +
    scale_x_log10() +
   ...
   ha <- HeatmapAnnotation(
      ...
      col = list(Group = structure(brewer.pal(length(unique_conditions), deseq2_color_palette), names = unique_conditions))
    )
...
return(list(deseq2_result_summary=text_deseq_summary, deseq2_result_significant=res_tax_sig, deseq2_result_table=res_tax, deseq2_normalized=normalized.counts, total_counts=parent_seq_count, plot=p1))
}
}
