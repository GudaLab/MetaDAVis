...
library("scales")
library("ggplotify")
metagenomeseq_plot <- function(OTU_input, group_index, index_pvalue, group1, group2, plot_method, alpha){
...
else if(group1!=group2)
{
  ...
  #print("No. significant were identified")
   ...
   p4<-Heatmap(df1, name = "Scale", show_row_names = TRUE, row_names_gp = gpar(fontsize = 5), show_column_names = TRUE, column_names_gp = gpar(fontsize = 5), show_row_dend = TRUE, show_column_dend = FALSE, cluster_columns = FALSE, top_annotation = ha)
   q <- as.ggplot(p4)
   q
    }
    }
