...
metagenomeseq_summary <- function(OTU_input, group_index, index_pvalue, group1, group2, plot_method, alpha, metagenomeseq_color_palette){
...
colors <- colorRampPalette(brewer.pal(9, metagenomeseq_color_palette))(length(unique_conditions))
...
if(plot_method == "1"){
   ...
   {
   p1 <- ggplot(data = res_tax, aes(x = All_Mean, y = log2FC, color = Significant)) +
    geom_point(size = 2) +
    scale_x_log10() +
   ...
   ha <- HeatmapAnnotation(
      ...
      col = list(Group = structure(brewer.pal(length(unique_conditions), metagenomeseq_color_palette), names = unique_conditions))
    )
   p4<-Heatmap(df1, name = "Scale", show_row_names = TRUE, row_names_gp = gpar(fontsize = 5), show_column_names = TRUE, column_names_gp = gpar(fontsize = 5), show_row_dend = TRUE, show_column_dend = FALSE, cluster_columns = FALSE, top_annotation = ha, col = colors)
   p1 <- as.ggplot(p4)
   #q
    }
return(list(metagenomeseq_result_summary=text_metagenomeseq_summary, metagenomeseq_result_significant=res_tax_sig, metagenomeseq_result_table=res_tax, total_counts=parent_seq_count, plot=p1))
}
}
