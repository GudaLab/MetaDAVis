...
anova_summary <- function(OTU_input, group_index, index_pvalue, ad_hoc,  plot_method, alpha, anova_color_palette){
unique_conditions <- unique(group_index$Condition)
colors <- colorRampPalette(brewer.pal(9, anova_color_palette))(length(unique_conditions))
...
text_anova_summary<- paste("Total of ", nrow(res_tax_sig), " taxa were identified as significant.", sep="")
  ...
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5))+
   ...
   ha <- HeatmapAnnotation(
      ...
      col = list(Group = structure(brewer.pal(length(unique_conditions), anova_color_palette), names = unique_conditions))
    )
...
return(list(anova_result_summary=text_anova_summary, anova_result_significant=res_tax_sig, anova_result_table=res_tax, anova_relative_frequency=rel_freq, total_counts=parent_seq_count, plot=p1))
