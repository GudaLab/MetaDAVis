...
library("scales")
library("ggplotify")
kruskal_wallis_test_plot <- function(OTU_input, group_index, index_pvalue, alpha, plot_method, ad_hoc){
...
text_kruskal_wallis_test_summary<- paste("Total of ", nrow(res_tax_sig), " taxa were identified as significant.", sep="")
  ...
  }
  {
   ...
   p3<-Heatmap(df1, name = "Scale", show_row_names = TRUE, row_names_gp = gpar(fontsize = 7), show_column_names = TRUE, column_names_gp = gpar(fontsize = 7), show_row_dend = TRUE, show_column_dend = FALSE, cluster_columns = FALSE, top_annotation = ha)
   q <- as.ggplot(p3)
   q
    }
	  }
