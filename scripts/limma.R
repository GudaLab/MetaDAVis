...
limma_summary <- function(OTU_input, group_index, index_pvalue, group1, group2, plot_method, alpha, limma_color_palette){
unique_conditions <- unique(group_index$Condition)
colors <- colorRampPalette(brewer.pal(9, limma_color_palette))(length(unique_conditions))
...
if(plot_method == "1"){
   ...
   ha <- HeatmapAnnotation(
      ...
      col = list(Group = structure(brewer.pal(length(unique_conditions), limma_color_palette), names = unique_conditions))
    )
  ...
  return(list(limma_result_summary=text_limma_summary, limma_result_significant=res_tax_sig, limma_result_table=res_tax, total_counts=parent_seq_count, plot=p1))
}
}
