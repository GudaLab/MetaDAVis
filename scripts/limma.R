...
limma_summary <- function(OTU_input, group_index, index_pvalue, group1, group2, plot_method, alpha){
{
if(group1==group2)
{
...
{ 
 group_index <- subset(group_index, ((group_index$Condition == group1) | (group_index$Condition == group2)))
  ...
  return(list(limma_result_summary=text_limma_summary, limma_result_significant=res_tax_sig, limma_result_table=res_tax, total_counts=parent_seq_count))
}
}
