...
ttest_summary <- function(OTU_input, group_index, index_pvalue, group1, group2, plot_method, alpha, ttest_color_palette){
...
if(group1==group2)
{
  ...
  text_ttest_summary<- paste("Please select different group in condition1 or condition2")
...
text_ttest_summary<- paste("Total of ", nrow(res_tax_sig), " taxa were identified as significant.", sep="")
}
else if(dim(res_tax_sig)[1]== 0)
  {
  text_ttest_summary<- paste("No. significant were identified. Adjust the FDR value or change the test correction method")
...
text_ttest_summary<- paste("Total of ", nrow(res_tax_sig), " taxa were identified as significant.", sep="")
  ...
  {
  text_ttest_summary<- paste("No. significant were identified. Adjust the Pvalue value or change the test correction method")
  #print("No. significant were identified")
}
}
}
...
colors <- colorRampPalette(brewer.pal(9, ttest_color_palette))(length(unique_conditions))
...
return(list(ttest_result_summary=text_ttest_summary, ttest_result_significant=res_tax_sig, ttest_result_table=res_tax, ttest_relative_frequency=rel_freq, total_counts=parent_seq_count, plot=p1))
}
}
}
